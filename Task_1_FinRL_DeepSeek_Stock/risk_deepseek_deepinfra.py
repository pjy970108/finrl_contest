...
model_used = 'risk_update_prompt_nan'  # Define the model_used variable
...
def get_sentiment(symbol, titles, *texts):
    ...
    conversation = [
        {"role": "system",
        "content": (
            f"Forget all previous instructions. You are now a financial expert specializing in risk assessment for stock recommendations. "
            f"I will provide recent news related to specific stock symbols. Based on these news articles, assess the risk level from 1 to 5, where: "
            f"{num_text} summarized news articles will be provided each time.\n\n"
            f"### **Guidelines:**\n"
            f"1 = very low risk, 2 = low risk, 3 = moderate risk (default if the news lacks clear risk indicators), "
            f"4 = high risk, and 5 = very high risk. "
            f"### **Special Considerations:**\n"
            f"- Does the news directly affect the company’s core business?\n"
            f"- Does the news indicate potential financial losses or legal risks?\n"
            f"- Is this a negative event that uniquely impacts this company compared to competitors?\n"
            f"- Is this issue short-term or long-term in nature?\n"
            f"- Is this news linked to macroeconomic trends (interest rates, inflation, regulations, etc.)?\n"
            
            f"Then, respond with a series of numbers separated by commas (e.g., '2,1,5'). Do not provide any additional text or explanations."
        )},
        {"role": "user",
         "content": f"News to Stock Symbol -- AAPL: Apple (AAPL) increase 22% ### News to Stock Symbol -- AAPL: Apple (AAPL) price decreased 30% ### News to Stock Symbol -- MSFT: Microsoft (MSTF) price has no change"},
        {"role": "assistant", "content": "5, 1, 3"},
...
if __name__ == "__main__":
    ...
    input_file_name='risk_nan_deepseek_nasdaq_news_data.csv'
