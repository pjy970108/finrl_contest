...
class Stock_Env(EnvConfig):
    ...
    def make_daily_data(self, transaction_fee, idx):
        cash_weight = 1 - sum(self.asset_weight_dict_list[idx].values())
        invest_weight = sum(self.asset_weight_dict_list[idx].values())
        ...
        for tic, weight in self.asset_weight_dict_list[idx].items():
            if weight == 0:
                continue
#            imp = daily_data_inx[daily_data_inx["tic"] == tic]
    ...
    def step(self, actions):
        ...
        for i in range(self.num_agents):
            ...
            imp, bench_mark = self.make_daily_data(total_transaction_fee, i)
            ...
            reward_dict[i] = {'return': returns, 'sharpe' : sharpe, 'sortino': sortino, 'calmar': calmar, 'mdd': mdd, 
