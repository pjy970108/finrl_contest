...
def main():
    ...
    with open("./data/train_daily.pkl", "rb") as f:
    ...
    feature_dim = 14
    train_scaled_tensor = torch.load("data/daily_data.pt")
    ...
    config.save_path = f'./TARN_MAPPO/model/scaled_data_actor_num_agent_{config.num_agents}_lr{config.lr_actor}_critic_lr{config.lr_critic}_gamma{config.gamma}_eps_clip{config.eps_clip}_reward_cond_{config.reward_cond}_entropy_weight{config.entropy_weight}_TARN_mappo_model.pth'
...
if __name__ == "__main__":
    ...
    SEED = 42
    set_seed(SEED)
    sweep_id = wandb.sweep(sweep_config, project="TARN-MAPPO-project_daily_contest_v1")
