...
from TARN_MAPPO.run_episode import *
import copy
from TARN_MAPPO.enviroment_episode import EnvConfig, Stock_Env
import wandb
import pickle
...
sweep_config = {
    ...
    "parameters": { 
        ...
        "reward_cond" : {"values": ["combined", "sharpe"]},
        # "entropy_weight" : {"values": [0.01, 0.1, 0.5]},
        "entropy_weight" : {"values": [0.5]},
        "K_epochs": {"values": [15, 20, 30]},
...
def main():
    ...
    config.save_path = f'./TARN_MAPPO/model/episode_high/risk_scaled_data_actor_num_agent_{config.num_agents}_lr{config.lr_actor}_critic_lr{config.lr_critic}_gamma{config.gamma}_eps_clip{config.eps_clip}_reward_cond_{config.reward_cond}_entropy_weight{config.entropy_weight}_k_epoch{config.K_epochs}_TARN_mappo_model.pth'
...
if __name__ == "__main__":
    ...
    SEED = 42
    set_seed(SEED)
    sweep_id = wandb.sweep(sweep_config, project="TARN-MAPPO-project_daily_episode_high_v1")
