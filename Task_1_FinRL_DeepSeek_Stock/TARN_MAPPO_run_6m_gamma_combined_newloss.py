...
#         "K_epochs": {"values": [10, 20, 30]} # Deep Reinforcement Learning Robots for Algorithmic Trading: Considering Stock Market Conditions and U.S. Interest Rates
...
sweep_config = {
    ...
    "parameters": { 
        ...
        "reward_cond" : {"values": ["combined_contest"]},
        # "lr_actor": {"values": [1e-3, 0.01, 0.1]},
        "lr_actor": {"values": [1e-4, 1e-3]},

        "lr_critic": {"values": [1e-5, 1e-4]},
        "gamma": {"values": [0.99, 0.98, 0.97]},
        # "gamma": {"values": [0.99]},

        "eps_clip": {"values": [0.1, 0.2, 0.3]},
        # "entropy_weight" : {"values": [0.01, 0.1, 0.5]},
        "entropy_weight" : {"values": [0.1, 0.01]},
        "num_agents": {"values": [4, 3, 2]},
        "K_epochs": {"values": [80, 50, 30]} # Deep Reinforcement Learning Robots for Algorithmic Trading: Considering Stock Market Conditions and U.S. Interest Rates
...
def main():
    ...
    with open("./data/train_daily_v4.pkl", "rb") as f:
        train_scaled = pickle.load(f)
    print("현재 작업 디렉토리:", os.getcwd())

    train_df = pd.read_csv("data/train_v4.csv", index_col=0)
    train_df.set_index("date", inplace=True)
    feature_dim = 15
    gpu_name = 1  # 0번 GPU를 의미

    if gpu_name is not None:
        torch.cuda.set_device(gpu_name)
        device = torch.device(f"cuda:{gpu_name}")
    else:
    ...
    train_scaled_tensor = torch.load("data/daily_data_v4.pt", map_location=device)
    ...
    # 경로 없으면 경로 생성하는 코드
    model_path = f'./TARN_MAPPO/model/{trajectory_lenght}/'
...
if __name__ == "__main__":
    ...
    SEED = 42
    set_seed(SEED)
    sweep_id = wandb.sweep(sweep_config, project="TARN-MAPPO-project_daily_6m_ippo")
